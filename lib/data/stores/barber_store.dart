...
class BarberStore {
  ...
  Future<void> updateCurrentBarberLocation({
    required double lat,
    required double lng,
  }) async {
    final barber = currentUserBarber;
  ...
  Barber _fromDb(Map<String, dynamic> row, List<BarberService> services) {
    return Barber(
      ...
      rating: double.tryParse(
            row['rating']?.toString() ?? row['puan']?.toString() ?? '0.0',
          ) ??
          0.0,
  ...
  Future<void> updateDaysOff(Set<int> daysOff) async {
    final barber = currentUserBarber;
    if (barber == null) return;
    await _client
        .from('barbers')
        .update({'days_off': daysOff.toList()})
        .eq('id', barber.id);
    await refreshBarbers();
  }
}
